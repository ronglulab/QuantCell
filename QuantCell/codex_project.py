...
class codex_project:
    ...
    """
            resolved=[]
            order_dict = {
                1: ['KLS', 'MEP', 'GMP', 'CLP', 'Erythroblasts', 'Erythrocytes', 'CMP', ],
                2: ['MkP'],
                3: ['Endothelial Niche'],
                4: ['CD4 T Cells', 'CD8 T Cells', 'Double Negative T Cells'],
                5: ['B Cells'],
                6: ['CFU-E'],
                7: ['Tregs'],
                8: ['Sinusoids'],
                9: ['Arteries', 'Capillaries'],
                10: ['Monocytes/Macrophages', 'Neutrophils', 'Perivascular Niche'],
                11: ['Preplatelets', 'cDC']
            }
            for label in orig_labs:
                if len(label) == 1:
                    resolved.append(label[0])
                else:
                    for tier in sorted(order_dict.keys()):
                        match=0
                        for cell in order_dict[tier]:
                            if cell in label:
                                match+=1
                        if match == 1: # if matches exactly one cell type in the tier, append that cell type
                            resolved.append([cell for cell in order_dict[tier] if cell in label][0])
                            break
                        if match > 1: # if matches more than one cell type in the tier, append 'Other', this happens if one of the cell types is not in the hierarchy
                            resolved.append('Other')
                            break
                    else:
                        resolved.append('Other')  # if no match found in any tier, append 'Other'

            return resolved
"""

def harmonize_codex_files(folder, marker_list):
    files = gb.glob(folder + '/*.csv')
    for file in files:
        file_name=file.split('/')[-1].split('.')[0]
        if ' (' in file_name:
            file_name=file_name.split(' (')[0]
        if file_name not in marker_list:
            print('Unknown marker name: ' + file.split('/')[-1][:-4] + ' in ' + folder)
            ...
            while change!='y' and change!='n':
                ...
                if change=='y':
                    ...
                    new_file_name=input()
                    os.rename(file, f'{folder}/{new_file_name} ({file_name}).csv')
                    print('File renamed')
                elif change=='n':
